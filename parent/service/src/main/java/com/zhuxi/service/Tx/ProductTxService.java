...
public class ProductTxService {
    ...
    public void putOnSale(Long productId){
        ...
        if (existsErrorSpec){
            throw new transactionalException(Message.EXISTS_ERROR_SPEC);
        }
        int i = productMapper.putOnSale(productId);
        if(i != 0)
            throw new transactionalException(Message.PUT_SALE_ERROR);
    }
    ...
    public List<SpecRedisDTO> getSpec(Long lastId,int pageSize){
        List<SpecRedisDTO> spec = productMapper.getSpec(lastId,pageSize);
        if(spec.isEmpty())
            throw new transactionalException(Message.SELECT_ERROR);
    ...
    @Transactional(readOnly = true,propagation=Propagation.SUPPORTS)
    public SpecRedisDTO getSpec(Long productId){
        SpecRedisDTO spec = productMapper.getSpecOne(productId);
        if(spec == null)
            throw new transactionalException(Message.SPEC_NOT_EXIST);
        return spec;
    }
    ...
    @Transactional(readOnly = true,propagation = Propagation.SUPPORTS)
    public ProductDetailVO getListProduct(Long lastId){
        ProductDetailVO product = productMapper.getListProduct(lastId);
        if(product == null)
            throw new transactionalException(Message.PRODUCT_ID_NOT_EXIST);
    ...
    public Integer updateBase(ProductBaseUpdateDTO productBaseUpdateDTO){
        ...
        if(productStatus == null)
            throw new transactionalException(Message.NO_PRODUCT);
