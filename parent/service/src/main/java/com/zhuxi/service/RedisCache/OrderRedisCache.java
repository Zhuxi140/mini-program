...
public class OrderRedisCache {
    ...
    private IdSnowFLake idSnowFLake;
    ...
    public void syncOrderData(List<OrderRedisDTO> orderRedisList,Long userId){
        ...
        redisUntil.executePipeline(p -> {
            ...
            orderRedisList.forEach(oRDto -> {
                ...
                objectObjectHashMap.put("groupId", oRDto.getGroupId());
                objectObjectHashMap.put("productName", oRDto.getProductName());
                objectObjectHashMap.put("specName", oRDto.getSpecName());
                objectObjectHashMap.put("coverUrl", oRDto.getCoverUrl());
                objectObjectHashMap.put("totalAmount", oRDto.getTotalAmount());
                objectObjectHashMap.put("status", oRDto.getStatus());
    ...
    /**
     * 获取SortSet表中第一个元素的 key
     * @param userId 用户id
     * @return SortSet表中第一个元素的 key
     */
    public Long getLastOrderId(Long userId){
        Set<Object> objects = redisUntil.ZSetRangeFirstElement(getOrderKeyByTime(userId));
        List<String> list = objects.stream()
                .map(Object::toString)
                .toList();
        Object orderId = redisUntil.hGet(getOrderDetailKey(list.get(0)), "id");

        return Long.parseLong((String) orderId);
    }
